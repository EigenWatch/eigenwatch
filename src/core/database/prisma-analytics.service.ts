...
import { Prisma, PrismaClient } from ".prisma/analytics-client";
...
{
  private readonly logger = new Logger(PrismaAnalyticsService.name);
  ...
  private async verifyConnection() {
    try {
      ...
      this.logger.log("Analytics database read-only verification passed");
    } catch (error) {
      this.logger.error("Analytics database verification failed", error);
    }
  }
